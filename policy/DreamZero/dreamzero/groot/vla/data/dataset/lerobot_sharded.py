...
class ShardedLeRobotSingleDataset(LeRobotSingleDataset):
    ...
    def get_all_parquet_paths(self) -> dict[int, Path]:
        ...
        if getattr(self, "is_lerobot_v3", False):
            return {
                int(trajectory_id): {
                    "path": self.get_parquet_path(int(trajectory_id)),
                    "offset": self._get_v3_video_time_offset(
                        self._get_v3_episode_meta()[int(trajectory_id)]
                    ),
                }
                for trajectory_id in self.trajectory_ids
            }
        return {trajectory_id: self.get_parquet_path(trajectory_id) for trajectory_id in self.trajectory_ids}
    ...
    def get_shard(
        ...
        for trajectory_id in trajectory_ids:
            ...
            parquet_ref = parquet_paths[trajectory_id]
            timestamp_offset = 0.0
            if isinstance(parquet_ref, dict):
                parquet_path = parquet_ref["path"]
                timestamp_offset = float(parquet_ref.get("offset", 0.0))
            else:
                parquet_path = parquet_ref
            if parquet_path in file_cache:
                parquet_df = file_cache[parquet_path]
            else:
                parquet_df = pd.read_parquet(parquet_path)
                file_cache[parquet_path] = parquet_df
            if "episode_index" in parquet_df.columns:
                parquet_df = parquet_df.loc[parquet_df["episode_index"] == trajectory_id].copy()
                parquet_df = parquet_df.sort_values("frame_index").reset_index(drop=True)
            if timestamp_offset:
                parquet_df["timestamp"] = parquet_df["timestamp"].astype(float) + timestamp_offset
