...
Two Unitree G1 robots; centralized (`env_cfg_type=unitree_g1x2_centralized`, one 70D policy over both robots' ego views stacked onto a 384x320 canvas) or decentralized (`unitree_g1x2_decentralized`, a 35D policy reading one robot's own 240x320 ego view). Decentralized is the benchmark's setting and it is **shared**: a single multitask policy, trained over all four tasks and both roles at once, drives both agents from one server and tells them apart by the instruction each is given. `MHBENCH_DECENTRALIZED_STYLE=per_robot` serves the older shape, one checkpoint per (task, robot). State is the raw joint-angle vector (86/43D), which the eval adapter reassembles exactly from `mhbench_state[*].joint_pos`; actions go out as `mhbench_raw_action` (the layout `MHBenchTaskEnv.take_action` requires -- DP's adapter is the reference).

Data for the shared policy comes from `baselines/scripts/prepare_multitask_data.sh fastwam` (all four tasks flattened into one dataset); a single-task run uses `baselines/scripts/prepare_fastwam_data.sh <task> <centralized|decentralized>`. Either writes `data/<dataset_id>/{lerobot,lerobot_val}` + `dataset_stats.json` (the fixed 50/10 split; the random `val_set_proportion` carve is off). Training goes through MHBench's `train_launch.sh FastWAM ...` or the standard chain (`bash train.sh mhbench <ckpt_name> <env_cfg_type> joint <seed> 0`); `train.sh` derives the task yaml, injects the state width, resumes from the newest saved trainer state and logs to wandb (`MHBench-FastWAM`, run id = run-dir name). Checkpoints land in `checkpoints/<run>/checkpoints/weights/step_*.pt`; serving picks the newest (or `checkpoint_num`). Eval: `eval_launch.sh FastWAM <task>` on the MHBench side (`serve/FastWAM.sh`).
