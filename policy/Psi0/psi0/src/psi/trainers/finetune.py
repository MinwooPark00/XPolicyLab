...
def _flash_attention_2_runs() -> bool:
    """Whether flash-attn is installed *and* has kernels for this card.

    `is_flash_attn_2_available()` answers the first half only. A wheel built
    before a GPU architecture existed imports cleanly and then dies at the first
    launch with "no kernel image is available" -- 2.7.4.post1, the version this
    repo pins, predates Blackwell. Running one tiny attention settles it in
    microseconds and cannot go stale the way a hard-coded list of supported
    compute capabilities would.
    """
    try:
        ...
        q = torch.zeros(1, 8, 1, 64, dtype=torch.bfloat16, device="cuda")
        flash_attn_func(q, q, q)
        torch.cuda.synchronize()
        return True
    except Exception as exc:  # not installed, no kernels, no CUDA
        overwatch.info(f"flash-attn unusable here ({type(exc).__name__}); using sdpa")
        return False
...
class FinetuneTrainer(Trainer):
    ...
    def init_qwen3vl_models(self):
        ...
        # MHBench: pick the backend rather than demand one. `flash_attention_2`
        # hard-coded here makes flash-attn a build-time requirement of *training*
        # -- a source build against the running torch -- while `models/psi0.py`,
        # which serves the same weights, already falls back to sdpa when
        # transformers reports it missing. Same rule on both sides; the model is
        # correct either way, flash-attn is the faster of the two.
        # PSI0_ATTN_IMPLEMENTATION forces one.
        attn_implementation = os.environ.get(
            "PSI0_ATTN_IMPLEMENTATION",
            "flash_attention_2" if _flash_attention_2_runs() else "sdpa")
        overwatch.info(f"Loading VLM weights in {dtype}, attention {attn_implementation}")
        try:
