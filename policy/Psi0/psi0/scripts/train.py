...
def _initialize_accelerator(trainer: Trainer) -> Accelerator:
    ...
    if overwatch.is_rank_zero() and not trainer.cfg.eval:
        ...
        if trainer.cfg.log.report_to == "wandb":
            ...
            # MHBench: honour an explicit --wandb.name. `WandbConfig.name` was a
            # dead field -- it is read out of cfg.wandb here and then overwritten
            # unconditionally, so no run could be named anything but the composed
            # <exp>.b<batch>.gpus<n>.<timestamp>. The benchmark names every
            # policy's run after its checkpoint directory so the four baselines
            # line up in one wandb view; run_name stays the fallback.
            wandb_config["name"] = wandb_config.get("name") or trainer.run_name
