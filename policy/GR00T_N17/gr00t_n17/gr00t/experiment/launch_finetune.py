...
if __name__ == "__main__":
    ...
    if os.path.isdir(cosmos_model) or os.environ.get("GR00T_COSMOS_MODEL"):
        config.training.transformers_local_files_only = True
    if get_rank() == 0:
        print(
            f"[launch_finetune] cosmos backbone: {config.model.model_name}, "
            f"local_files_only={config.training.transformers_local_files_only}"
        )
