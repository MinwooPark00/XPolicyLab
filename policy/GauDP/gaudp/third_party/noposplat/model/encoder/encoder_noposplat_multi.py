...
class EncoderNoPoSplatMulti(Encoder[EncoderNoPoSplatCfg]):
    ...
    def forward(
    ...
    ) -> Gaussians | tuple[Gaussians, Tensor]:
        ...
        if not return_features:
            return flat_gaussians

        if self.cfg.num_surfaces != 1 or self.cfg.gaussians_per_pixel != 1:
            raise ValueError("GauDP's pixel-aligned feature path requires one Gaussian per pixel")
        means = rearrange(gaussians.means, "b v (h w) 1 1 xyz -> b v xyz h w", h=h, w=w)
        covariances = rearrange(
            gaussians.covariances,
            "b v (h w) 1 1 i j -> b v (i j) h w",
            h=h,
            w=w,
        )
        opacities = rearrange(gaussians.opacities, "b v (h w) 1 1 -> b v 1 h w", h=h, w=w)
        return flat_gaussians, torch.cat((means, covariances, opacities), dim=2)
