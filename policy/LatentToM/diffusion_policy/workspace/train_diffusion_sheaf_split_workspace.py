    ...
    def run(self):
        ...
        if cfg.training.resume:
            ...
            if arm1_ckpt_path.is_file():
                ...
                self.load_checkpoint(path=arm1_ckpt_path)
            if arm2_ckpt_path.is_file():
                print(f"Resuming arm2 from checkpoint {arm2_ckpt_path}")
                self.load_checkpoint(path=arm2_ckpt_path)
        ...
        # xarm_image_runner.XarmImageRunner); the eval side of this adapter
        # runs through the client-server loop (eval.sh) instead, same as
