...
class DiffusionSheafSplitPolicy(BaseImagePolicy):
    def __init__(self,
        ...
        # get feature dim
        private_dim_per_step = obs_encoder.output_shape()[0][0]  # 1031 for decentralized
        sheaf_embedding_dim = obs_encoder.output_shape()[1][0]
        # new obs combined feature dim
        obs_feature_dim = private_dim_per_step + sheaf_embedding_dim
        ...
        # for loss
        # compute_loss() reshapes compute_loss's per-step [B*To, D] encoder
        # output to [B, To*D] before returning it as sheaf_embedding/private_
        # embedding (see obs_as_global_cond branch), so ToMCrossPredictor's
        # input dims are each per-step dim * n_obs_steps -- not
        # ToMCrossPredictor's own (task-specific) defaults, which were sized
        # for the original xarm task's shape_meta and silently mismatch any
        # other one (obs_feature_dim/global_cond_dim above already derive
        # from obs_encoder.output_shape() the same way; this just does the
        # same for the class ToMCrossPredictor doesn't compute on its own).
        self.tom_predictor = ToMCrossPredictor(
            shared_dim=sheaf_embedding_dim * n_obs_steps,
